...
//对vector的shell排序
void shellsort(vector*& L)
{
	int i, j, temp, gap = L->length;
	while (gap > 1)
	{
		gap = gap / 2;/*增量缩小，每次减半*/
		for (i = 0; i < L->length - gap; i++) // n - gap是控制上限不让越界
		{
			j = i + gap;
			if (L->data[i] > L->data[j])// 相邻间隔的前后值进行比较
			{
				temp = L->data[i];
				L->data[i] = L->data[j];
				L->data[j] = temp;
...
int main(void)
{
	...
	printf("\n  请选择需要执行的操作：\n  1.建立vector  2.销毁vector  3.对vector进行shell排序  4.输出vector  （输入q退出）\n");
	while (scanf("%d", &num) != 0)
	{
		switch (num)
		{
		...
		case 3:for (i = 1; i < 10;i++)shellsort(L); printf("\n  shell排序完成！\n"); break;
		case 4:printf("\n  vector元素如下：\n"); displist(L); break;
		default: printf("\n  输入错误！\n");
		}
		printf("\n  请选择需要执行的操作：\n  1.建立vector  2.销毁vector  3.对vector进行shell排序  4.输出vector  （输入q退出）\n");
